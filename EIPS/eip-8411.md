...
`bid_root` is the `hash_tree_root` of the `ExecutionPayloadBid` whose `payload_chunks_root` commits to the chunk set. Because bids propagate on their own gossip topic before the beacon block, `bid_root` lets a receiver verify a chunk as soon as it has seen the corresponding bid, without waiting for the block.

`slot` and `beacon_block_root` sit outside the inclusion proof and are unauthenticated hints: `slot` allows windowing a chunk before its bid is known and MUST equal `bid.slot`, while `beacon_block_root` lets a receiver locate or request the corresponding beacon block and MUST equal the root of the block containing the bid.
...
The `Seen` cache used by the gossip validation functions is extended to retain observed bids by root and to track received chunks:

```python
@dataclass
class Seen:
    ...
    execution_payload_bids_by_root: Dict[Root, ExecutionPayloadBid]  # [New in this EIP]
    payload_chunk_tuples: Set[Tuple[Root, ChunkIndex]]  # [New in this EIP]
```

An entry keyed by `hash_tree_root(bid)` is added to `execution_payload_bids_by_root` for every bid whose `SignedExecutionPayloadBid` is accepted on the bid gossip topic, and for the bid contained in every valid beacon block. Entries MUST be retained at least until the end of `bid.slot` (within `MAXIMUM_GOSSIP_CLOCK_DISPARITY`) and MAY be pruned afterwards.

This is a new retention requirement. The existing `execution_payload_bids` set records only identifying tuples, not bid contents, since under EIP-7732 the winning bid arrives inside the beacon block. Under this EIP chunk validation must resolve `chunk.bid_root` against any valid bid for the current slot before the block is known. The cache stays small: the bid topic's own validation rules bound how many bids enter it per slot.

`payload_chunk_tuples` records the `(bid_root, index)` pair of every accepted chunk, analogous to `data_column_sidecar_tuples` for column sidecars, and backs the first-seen check below.

#### Gossip validation

The following validations MUST pass before a node forwards a `PayloadChunk` `chunk` on the topic. `bid` denotes `seen.execution_payload_bids_by_root[chunk.bid_root]`.
...
- _[IGNORE]_ `chunk.bid_root` is present in `seen.execution_payload_bids_by_root`. Clients MAY queue chunks referencing an unknown `bid_root` for a short period and revalidate once the bid arrives.
- _[REJECT]_ `chunk.slot` equals `bid.slot`.
- _[REJECT]_ `chunk.chunk_inclusion_proof` verifies `chunk.chunk` as the leaf at index `chunk.index` of the Merkle tree with root `bid.payload_chunks_root`.
- _[REJECT]_ If the node has seen a valid beacon block whose bid message hashes to `chunk.bid_root`: `chunk.beacon_block_root` equals the hash tree root of that block.
- _[IGNORE]_ `(chunk.bid_root, chunk.index)` is not present in `seen.payload_chunk_tuples` (first-seen per bid and index); the tuple is added upon acceptance.
...
Chunks are not individually signed; their authenticity derives entirely from the bid signature via `payload_chunks_root`. `bid_root` is itself unauthenticated, but it serves only as a lookup key into bids that have already passed signature validation: a chunk referencing an unknown `bid_root` is not forwarded, so unsigned chunk spam is filtered at the first hop.

`slot` and `beacon_block_root` are covered by neither the bid signature nor the inclusion proof and MUST NOT be treated as trust inputs; they only route lookups. The consistency checks in gossip validation bound their misuse: a chunk replayed with a divergent `slot` is rejected against `bid.slot`, and a divergent `beacon_block_root` is rejected once the block containing the bid is known. Because the chunk bytes are proof-bound, a message that differs only in these hint fields cannot alter the reconstructed envelope.

Per-index equivocation is not possible: two distinct chunks that both verify at the same `(bid_root, index)` would imply a collision in the Merkle tree, so at most one chunk per index can propagate for a given bid.

Pre-block buffering is bounded: a node stores at most `TOTAL_EXECUTION_PAYLOAD_CHUNKS` chunks per entry in `execution_payload_bids_by_root`, and the size of that cache is itself limited by bid gossip validation (builder signature and slot window).
