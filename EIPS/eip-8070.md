...
- **P1 (recent inclusions):** Protects a fraction (e.g., top 10%) of inbound and dialed peers based on an Exponential Moving Average (EMA) of their per-block transaction inclusion contributions. 
  - For each new block, a peer's score is updated as: `score = (1 - alpha) * score + alpha * blockInclusions`, where `alpha = 0.05` and `blockInclusions` is the number of transactions the peer delivered that were included in the newly arrived block.
  - This allows newly connected but highly productive peers to earn protection quickly and naturally decays the score of inactive peers toward zero.

**Adversarial Analysis**

| Category | Gameability | Rationale |
|---|---|---|
| Recent Inclusions (EMA) | **Hard** | Requires actual block production; cannot be faked without real economic cost. |
