...
Reference implementation evidence is available in the simulator suite in this repository.

### Heuristic simulator (single-node) results

The following command runs a mixed-adversary scenario (spammer, withholder, spoofer,
free-rider, non-announcer, selective signaler) and prints per-heuristic outcomes:

```bash
uv run python -m heuristic_sim.sim --seed 42 --t-end 300
```

Observed output (seed 42):

- H1 rejected 4,486 low-fee spam announcements.
- H2 evicted 18,255 selectively-signaled transactions lacking independent corroboration.
- H4 disconnected all spoofers/withholders in the run (spoofer: 2/2, withholder: 3/3).
- H5 disconnected request-heavy low-contribution peers (free_rider: 2/2, non_announcer: 2/2).
- Honest peers were not disconnected (false positives: 0/30).

Parameter sensitivity can be reproduced with:

```bash
uv run python -m heuristic_sim.sweep
```

In the default sweep scenario, detection remained stable for H4/H5 across tested ranges,
while H1/H2 activity shifted with `includability_discount` and `saturation_timeout`.
...
### Peer protection heuristics (P1-P2)

To avoid creating unnecessary network churn and to preserve high-value connections, nodes SHOULD implement heuristics to shield productive peers from random disconnection. Inclusion-based heuristics are RECOMMENDED as they are grounded in on-chain evidence.

- **P1 (total inclusions):** protect peers based on the cumulative count of unique transactions they delivered that were successfully included in the canonical chain. This favors long-lived, consistently productive peers.
- **P2 (recent inclusions):** protect peers based on an Exponential Moving Average (EMA) of their per-block inclusion contributions. This allows newly connected but highly productive peers to earn protection quickly and decays the score of inactive peers.

**Adversarial Analysis**

| Category | Gameability | Rationale |
|---|---|---|
| Total Inclusions | **Hard** | Requires actual block production; cannot be faked without real economic cost. |
| Recent Inclusions (EMA) | **Hard** | Same as above; requires continuous block production to maintain protection. |

### Non-normative local heuristic profile (H1-H5)

The protocol intentionally leaves local anti-abuse policy to implementations. One
concrete profile used in this repository is:

- **H1 (includability check):** reject announcements with fee below
  `blob_base_fee * includability_discount`.
- **H2 (saturation check):** after `saturation_timeout`, evict transactions not
  independently announced by at least `min_independent_peers`.
- **H3 (sampling noise probes):** when requesting custody columns, also request
  random extra columns (`C_extra`) to test whether an announcing provider actually
  serves beyond predictable custody indices.
- **H4 (random-column failure):** disconnect peers whose non-custody probe failure
  rate exceeds a local threshold over a tracking window.
- **H5 (request/announcement fairness):** disconnect peers whose inbound request
  volume is disproportionately high relative to their announcement contribution.

This profile is informative only. It is not a protocol requirement and is not
intended to standardize scoring formulas or eviction thresholds.
...
The heuristic simulator bundled with this repository includes one experimental scoring
model for research and tuning. Its results are operational guidance only and do not
alter the non-normative status of peer scoring in this EIP.
